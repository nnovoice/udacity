...
def ancestors(genealogy, person):
    if person not in genealogy:
        ...
        ancestors_list = genealogy[person]
        for ancestor in ancestors_list:
            more_ancestors = ancestors(genealogy, ancestor)
            if (more_ancestors != None):
                for person in more_ancestors:
                    if person not in ancestors_list:
...
print ancestors(ada_family, 'Anne Isabella Milbanke')

print ancestors(ada_family, None)

print ancestors(ada_family, 'Judith Blunt-Lytton')
for person in ada_family:
    print ancestors(ada_family, person)
